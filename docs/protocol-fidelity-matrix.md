...
| GenUI action callback transport | ✅ | `submitGenUIAction` now uses capability-negotiated ACP/Codex action routes with protocol metadata (`surfaceId`, `revision`, `correlationId`, `context`). |
| GenUI action capability negotiation | ✅ | Initialize capability parsing now selects a single protocol-native action route (no multi-method probing). |
...
| GenUI revision ordering | ✅ | Incoming stale revisions are ignored to prevent patch regressions. |
...
| Expanded GenUI action callback contracts | Improved (not complete) | Revision/correlation/context callback fields normalized; advanced server-specific callbacks still pending |
