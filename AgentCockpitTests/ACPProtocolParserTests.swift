...
final class ACPProtocolParserTests: XCTestCase {
    ...
    func testParseSessionUpdateToolCallAndUpdate() {
        ...
        XCTAssertEqual(updateContext.toolStatus, nil)
    }

    func testParseSessionUpdateChunkKindsAndStatusExtraction() {
        let userChunk = ACPProtocolParser.parseSessionUpdate(
            params: [
                "sessionId": AnyCodable("session-1"),
                "update": AnyCodable([
                    ...
                    "content": AnyCodable([
                        ...
                        "text": AnyCodable("hello chunk")
                    ])
                ])
            ],
            fallbackSessionID: nil
        )
        XCTAssertEqual(userChunk.type, .userMessage)
        XCTAssertEqual(userChunk.text, "hello chunk")

        let toolUpdate = ACPProtocolParser.parseSessionUpdate(
            params: [
                "sessionId": AnyCodable("session-1"),
                "update": AnyCodable([
                    ...
                    "result": AnyCodable("working")
                ])
            ],
            fallbackSessionID: nil
        )
        XCTAssertEqual(toolUpdate.type, .toolCallUpdate)
        XCTAssertEqual(toolUpdate.toolStatus, "in_progress")
        XCTAssertFalse(toolUpdate.isError)
    }
