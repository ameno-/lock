...
final class CodexProtocolParserTests: XCTestCase {
    ...
    func testParseThreadReadsThreadIDEnvelopeWithoutNestedThread() {
        let payload = AnyCodable([
            "threadId": AnyCodable("thread-started"),
            "status": AnyCodable([
                "type": AnyCodable("idle")
            ]),
            "cwd": AnyCodable("/Users/ameno/dev/zed-dev/agentcockpit"),
            "updatedAt": AnyCodable(1_772_221_307)
        ])
        ...
        XCTAssertEqual(thread?.id, "thread-started")
        XCTAssertEqual(thread?.statusType, "idle")
        XCTAssertEqual(thread?.cwd, "/Users/ameno/dev/zed-dev/agentcockpit")
        XCTAssertEqual(thread?.isRunning, false)
